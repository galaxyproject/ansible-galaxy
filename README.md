...
- `galaxy_config_files`: List of hashes (with `src` and `dest` keys) of files to copy from the control machine. To set destinations, you can use the `galaxy_config_dir` variable followed by the file name. Make sure to add the appropiate setup within `galaxy_config` for each file added here (so, if adding `tool_conf.xml` make sure that `galaxy_config.galaxy.job_config_file` points to that file).
